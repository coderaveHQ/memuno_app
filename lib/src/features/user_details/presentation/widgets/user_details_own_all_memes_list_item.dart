...
/// Card widget for rendering one own\-all user-details meme item.
class UserDetailsOwnAllMemesListItem extends ConsumerWidget {
  /// Creates one own\-all user-details meme card.
  const UserDetailsOwnAllMemesListItem({super.key, required this.item});

  /// own\-all list payload to render.
  final UserDetailsOwnAllMemesListPageItemEntity item;
  ...
  Future<void> _onToggleLaugh(WidgetRef ref) async {
    ...
    await mutation.runSafely(ref, (MutationTransaction tx) async {
      await ref
          .read(userDetailsOwnAllMemesListProvider.notifier)
