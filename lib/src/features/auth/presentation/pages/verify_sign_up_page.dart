...
import 'package:memuno_app/src/features/auth/application/mutations/resend_sign_up_otp_mutation.dart';
import 'package:memuno_app/src/features/auth/application/mutations/verify_sign_up_otp_mutation.dart';
import 'package:memuno_app/src/features/auth/application/providers/usecases/resend_sign_up_otp_usecase_provider.dart';
import 'package:memuno_app/src/features/auth/application/providers/usecases/verify_sign_up_otp_usecase_provider.dart';
...
/// Verification page shown after sign up.
class VerifySignUpPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    ref.listen<MutationState<void>>(resendMutation, (previous, next) {
      if (next is MutationError<void>) {
        ...
        feedback.showSuccess(
          context,
          message: l10n.verifySignUpResentCodeMessage,
        );
      }
    });
    ...
    return Scaffold(
      appBar: AppAppBar(
        title: l10n.verifySignUpTitle,
        subtitle: l10n.verifySignUpInstruction(email),
        onBack: () => context.pop(),
      ),
      body: Center(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    AppPinput(
                      ...
                      length: _otpLength,
                      labelText: l10n.verifySignUpCodeLabel,
                    ...
                    AppButton.primary(
                      onPressed: canSubmit
                          ...
                          : null,
                      isLoading: isLoading,
                      child: Text(l10n.verifySignUpConfirmButton),
                    ),
                    AppGap.v(spacing.md),
                    Text(
                      l10n.verifySignUpNoEmailText,
                    ...
                    AppButton.ghost(
                      ...
                      child: Text(l10n.verifySignUpResendCodeButton),
                    ),
                  ],
                ),
              ),
  ...
  Future<void> _submitVerification(
    ...
    final Mutation<void> mutation = ref.read(verifySignUpOtpMutationProvider);
    await mutation.runSafely(ref, (MutationTransaction tx) async {
      final VerifySignUpOtpUsecase usecase = tx.get(
        verifySignUpOtpUsecaseProvider,
      );
  ...
  Future<void> _resendOtp(WidgetRef ref, String email) async {
    final Mutation<void> mutation = ref.read(resendSignUpOtpMutationProvider);
    await mutation.runSafely(ref, (MutationTransaction tx) async {
      final ResendSignUpOtpUsecase usecase = tx.get(
        resendSignUpOtpUsecaseProvider,
      );
      await usecase(
        email: email.trim(),
